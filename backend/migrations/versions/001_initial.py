...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "typeof(amount) = 'integer' AND amount > 0 AND amount <= 99999999999999", name="ck_amount_cents"
        ),
        sa.CheckConstraint("type IN ('INCOME', 'EXPENSE')", name="ck_type"),
        sa.CheckConstraint("status IN ('PAID', 'PENDING')", name="ck_status"),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            server_default=sa.text("CURRENT_TIMESTAMP"),
